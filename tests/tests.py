...
from my_module import ascii_encode_seqs

def test_ascii_encode_seqs():
    # Test 1 - Check if the returned array has the expected shape and datatype
    seqs = ['abc', 'def', 'ghi']
    pad = 2
    expected_output = np.array([[97, 98, 99, 0, 0], [100, 101, 102, 0, 0], [103, 104, 105, 0, 0]])
    output = ascii_encode_seqs(seqs, pad=pad)
    assert output.shape == expected_output.shape, f"Expected shape {expected_output.shape}, but got {output.shape}"
    assert output.dtype == expected_output.dtype, f"Expected dtype {expected_output.dtype}, but got {output.dtype}"
