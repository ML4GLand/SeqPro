...
- **`AminoAlphabet.translate(seqs, ..., validate=False, on_unknown="error", unknown_marker="X")`**: translates nucleotides → amino acids. Lowercase nucleotides translate identically to uppercase (case-insensitive — soft-mask is annotation, not biology). Use `on_unknown` to choose how non-canonical codons (any byte outside the alphabet, e.g. `N`, NUL, IUPAC codes) are handled: `"error"` (default — raises with codon position), `"pad"` (emit one `unknown_marker` per unknown codon, same shape as input), `"collapse"` (emit one marker per consecutive run of unknown codons — Ragged input only), `"shorten"` (drop unknown codons — Ragged input only). `unknown_marker` is a single ASCII byte (default `"X"`). The orthogonal `validate=True` flag still rejects any byte outside the alphabet (including lowercase, because the alphabet itself is uppercase) — opt-in strict input checking, independent of `on_unknown`.
